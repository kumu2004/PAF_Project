...
import { ArrowLeft, AlertCircle, Save, Sparkles, Loader2 } from 'lucide-react';
...
const EditTicketPage = () => {
  ...
  const { data: ticket, isLoading: isFetching } = useTicket(id);
  const { updateTicket, isLoading: isUpdating } = useTicketActions();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateTicket({ id, data: formData });
      navigate(`/tickets/${id}`);
    } catch (error) {
      ...
      <Card className="border-none shadow-2xl shadow-primary/5">
        <CardHeader className="space-y-4 pb-8 border-b bg-muted/5">
          ...
          <div className="space-y-1">
            <CardTitle className="text-3xl font-black tracking-tight">Edit Ticket</CardTitle>
            <CardDescription className="text-base">
              You are modifying ticket <span className="font-bold text-primary">#{id?.substring(0, 8)}</span>
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent className="pt-8">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-3">
              ...
              <Input 
                name="resourceId"
                placeholder="Equipment ID or Room Number" 
            ...
            <Button type="submit" className="w-full h-14 text-lg font-bold shadow-xl shadow-primary/20" disabled={isUpdating}>
              {isUpdating ? (
                ...
                <span className="flex items-center gap-2">
                   Save Updates <Save className="w-5 h-5" />
                </span>
              )}
            </Button>
          </form>
